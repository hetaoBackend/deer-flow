...
```text
https://raw.githubusercontent.com/bytedance/deer-flow/main/Install.md に従って DeerFlow のローカル開発環境をセットアップしてください
```

このプロンプトは coding agent 向けです。Docker が使える場合は Docker を優先し、リポジトリの初期セットアップを行ったうえで、最後に次の起動コマンドと不足している設定項目だけを返します。
