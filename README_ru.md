...
```text
Помоги настроить локальное окружение разработки DeerFlow по инструкции https://raw.githubusercontent.com/bytedance/deer-flow/main/Install.md
```

Этот prompt предназначен для coding agent. Он просит агента предпочесть Docker, если он доступен, подготовить репозиторий и в конце вернуть точную команду запуска и список недостающих настроек.
